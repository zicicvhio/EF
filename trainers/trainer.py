...
class Trainer(object):
    ...
    def compute_losses(self, pred, batch, records, stage='train'):
        ...
        info.append('Loss: {:.3f} ({:.3f})'.format(records['total'].val,
                                                    records['total'].avg))
        info = '\t'.join(info)
        step = self.step[stage]
    ...
    def train(self, epoch):
        ...
        for i_batch, batch in enumerate(self.train_loader):
            ...
            # #print information during training(hqf)
            if i_batch == num_iters-1 or i_batch==1:
            ...
            # # print information during training(REDS)
            # records['time'].update(time.time() - start_time)
            # info = 'Epoch: [{}][{}/{}]\t' \
            #        'Time: {:.3f} ({:.3f})\t{}'.format(epoch,
            #                                           i_batch,
        ...
        # print('Epoch: [{}]\tTotal: {:.3f} ({:.3f})'.format(epoch,
        #                                                    records['total'].val,
        #                                                    records['total'].avg))
