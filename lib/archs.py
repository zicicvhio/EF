...
class UKAN(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        self.encoder1 = ConvLayer(27, kan_input_dim // 8)
    ...
    def forward(self, x):
        ...
        ### Tokenized KAN Stage
        ### Stage 4
        # print("开始out", out.shape)

        out, H, W = self.patch_embed3(out)

        # print("之后out", out.shape)
        # print("之后H", H)
        # print("之后W", W)
        ...
        out = F.interpolate(out, size=(180, 240), mode='bilinear', align_corners=False)

        return self.final(out)

if __name__ == "__main__":
    dim = 27
    block = UKAN(num_classes=256)
    input = torch.randn(6, dim, 180, 240)
    input = F.interpolate(input, size=(224, 256), mode='bilinear', align_corners=False)
    output = block(input)
